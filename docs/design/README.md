...
**Text**: header `Text` + line id, and on the right three icon buttons that copy the original, the translation and the prompt. Two blocks, `ORIGINAL` and `TRANSLATION`, each with a color square, language and length on the right (`EN · 8.20s`, `UK · 84`). Whole text in 17px, editable as one block. Underline (2px accent) marks the part the next Generate produces. Generator grid below in four columns: Voice, a `Generate | Record` mode switch, then the action for the active mode — the Generate split button with the character cost and the remaining balance above it in mono, or the Record button. Under a hairline and the `ELEVENLABS` group label sit the settings of the active mode: Model, Language where the model takes one, then only the voice settings the model accepts (Stability, Similarity, Style, Speed, Boost) as numeric fields and a toggle. A v3 model adds a row of audio-tag chips that insert `[whispers]`, `[laughs]` and the rest at the caret of the translation.
