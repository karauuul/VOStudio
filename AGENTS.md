...
3. **No new runtime dependencies.** DSP is hand-written on Web Audio/AudioWorklet. Worklet code ships as an embedded string via Blob URL (see `effect-worklets.ts`) — `?url`/`?worker` imports break under `file://` inside asar.
