...
- `src/main/` — Electron main process: project store (`project-store.ts`, atomic writes, autosave, versions), IPC handlers and zod validation (`index.ts`, `schemas.ts`), export encoding and video mux (`export.ts`, `ffmpeg.ts`), audio, table and long-source import (`audio-import.ts`, `table-import.ts`, `sources.ts`, `template-import.ts`), provider adapters (`providers/elevenlabs.ts`), API keys via `safeStorage` (`secrets.ts`).
...
- `src/renderer/` — React UI and audio engine.
  ...
  - Import room: `import/SourcesPanel.tsx`, `import/LinesTable.tsx`, `import/ProjectPanel.tsx`; `import/TableImportDialog.tsx` (table and script preview with column mapping, opened from Import and Work). Export room: `export/OutputPanel.tsx`, `export/ReadinessTable.tsx`, `export/SummaryPanel.tsx`, `export/run-export.ts` (`runJob` / `runPlan`).
